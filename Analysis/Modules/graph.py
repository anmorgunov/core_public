...
class Graph:
    ...
    def _save_fig(self, figure, fname, html=True, jpg=True, svg=True):
        if html: figure.write_html(tools.join_path(self.PAPER+['output', 'figures', 'html', f"{fname}.html"]), include_plotlyjs='cdn')
        if jpg: figure.write_image(tools.join_path(self.PAPER+['output', 'figures', 'jpg', f"{fname}.jpg"]), scale=4.0)
        if svg: figure.write_image(tools.join_path(self.PAPER+['output', 'figures', 'svg', f"{fname}.svg"]))