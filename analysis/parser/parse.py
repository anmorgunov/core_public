...
class ParserObject:
    ...
    def __init__(self, path, file, experFile, methodToFolders):
        """
        ...
        Note that path and file are combined immediately after receiving. In sum, there isn't much reason to store them as separate variables, but I imagine it could be handy at some point in future
        """
        self.BASE = path
        self.SUBMODULE = self.BASE + ["analysis", "parser"]
        self.CALCULATIONS = self.BASE + ["calculations"]
        self.readWB = load_workbook(tools.join_path(self.SUBMODULE+[file,]))
            ...
            for folder in folders:
                ...
                for molecule in molecules: 
                    ...
                    for basis in bases:
                        ...
                        files = os.listdir(tools.append_path(molPath, [molecule, basis], endWithSlash=True)) 
                        submitScript = open(tools.append_path(molPath, [molecule, basis, "submit.sh"]), 'r')
                        ...
                        if f'CEBE_{method}.txt' not in files:
                            ...
                            errFile = open(tools.append_path(molPath, [molecule, basis, "sbatch.err"]), 'r').readlines()
                            if errFile:
                                molToData[molecule][basis]['detailed'] = errFile[-1].split('\n')[0]
                            else:
                                molToData[molecule][basis]['detailed'] = 'empty?'
                        else:
                            with open(tools.append_path(molPath, [molecule, basis, f"CEBE_{method}.txt"]), 'r') as f:
                                lines = f.readlines()
    ...
    def _parse_experimental(self):
        ...
        """
        source = tools.join_path(self.SUBMODULE + [self.experFile,])
    ...
    def write_results(self):
            ...
            for molecule, basToData in molToData.items():
                for basis, data in basToData.items():
                    ...
                    if 'error' in data:
                        ...
                        if 'UHF' in data: ws['F'+str(r)] = data['UHF']
                        if 'MP2' in data: ws['G'+str(r)] = data['MP2']
                        if 'MP3' in data: ws['H'+str(r)] = data['MP3']
                        if 'CCSD' in data:
                        ...
                        if 'Frozen orbitals' in data: ws['L'+str(r)] = data['Frozen orbitals']
                        if 'Swapped orbitals' in data: ws['M'+str(r)] = data['Swapped orbitals']
                        if 'T1 for RHF' in data: ws['N'+str(r)] = round(float(data['T1 for RHF']), 4)
                        if 'T1 for UHF(a)' in data: ws['O'+str(r)] = round(float(data['T1 for UHF(a)']), 4)
                        if 'T1 for UHF(b)' in data: ws['P'+str(r)] = round(float(data['T1 for UHF(b)']), 4)
        ...
        self.saveWB.save(tools.join_path(self.SUBMODULE+[self.readFile,]))
    ...
    def create_mol_objects(self):
        for molPath in self.molPaths:
            molecule = tools.join_path(self.CALCULATIONS + ['mols', 'exp'], endWithSlash=True) + molPath + '.xyz'
    ...
    def wrapper(self, fillMol=False):
        ...
        self.write_results()
        if fillMol: self.create_mol_objects()
        self.create_mp25()

def perform_parsing(fillMol=False):
    ...
    resObj = ParserObject(PATH, WB, EXPER, METHOD_TO_FOLDERS)
