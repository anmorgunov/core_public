        ...
        if isCore and polarization in zetaToPolToNum[zeta]:
            ...
            while True:
                ...
                if all(coeff[-1] == '0.000000e+00' for coeff in array2D):
                    ...
                    no_zeros = "\n".join(['           '.join(coeff) for coeff in no_zeros_array])
                    n_coeff = no_zeros
                else:
                    break
            # print(no_zeros_array)
            ...
            for i in range(exclude):
                ...
                no_zeros = '           '.join([coeff for coeff in i_coeff.split() if coeff != '0.000000e+00'])
                # print(i_coeff)
                # print(no_zeros)
                out += f"{element} {polarization}\n{no_zeros}\n"
        else:
...
def compose_basis_set(zeta):
    ...
    with open(tools.join_path(BASE+[SUBMODULE ,'nwchem', f'cheng{zeta}.py']), 'w') as f:
        f.write("BASIS_SET = ")
