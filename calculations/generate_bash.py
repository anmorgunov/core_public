...
class SubmitGenerator:
    ...
    def process_bases(self, bases):
        ...
        regList = [
            zetaToBasis[basis][0] if basis in zetaToBasis else basis
            for basis in basesList
        ]
        coreList = [
            zetaToBasis[basis][1] if basis in zetaToBasis else basis
            for basis in basesList
        ]
    ...
    def create_scripts(self, params):
        ...
        cluster = memToCluster[
            max([mem for mem in memToCluster.keys() if int(memory) >= mem])
        ]  # select a cluster that corresponds to a largest key s.t. the key is smaller than requirement
        ...
        for molList, doLoc in ((mols, False), (molsLoc, True)):
            ...
            filename = (
                scripts_directory / f"generated_{atom}_{self.cntr}{suffix[doLoc]}.sh"
            )
