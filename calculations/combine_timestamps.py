...
def create_new_timestamps(path, times):
    if "raw_timestamps.txt" not in (
        os.listdir(tools.join_path(path, endWithSlash=True))
    ):
        os.rename(
            tools.join_path(path + ["timestamps.txt"]),
            tools.join_path(path + ["raw_timestamps.txt"]),
        )
    filestr = """"""
    for method in "HF MP2 MP3 CCSD CCSD(T) UHF UMP2 UMP3 UCCSD UCCSD(T)".split():
        if path[-1] == "Q" and method in {"MP3", "UMP3"}:
            continue
        if path[-1] == "5" and method not in {"HF", "UHF", "MP2", "UMP2"}:
            continue
        filestr += f"- {method}\n"
