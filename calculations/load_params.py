class ParamParser:
    ...
    def _parse(self):
        ...
        for i, rline in enumerate(self.lines):  # for each line in launch_parameters.txt
            ...
            if (
                not line or line[0] == "#"
            ):  # if an empty line or line starts with a # we ignore it
                continue

            if (
                line == "START"
            ):  # if we found a line that says "START", we start parsing
                for j in range(
                    i, len(self.lines)
                ...
                ), "Launch_parameters.txt contains a block without END"  # the block must end somewhere

                launchParamSet = {}
                for paramIndex in range(
                    i + 1, endIndex
                ...
                assert (
                    "atom" in launchParamSet
                ), "You must specify an atom on which a core orbital will be ionized"
                assert (
                    "mols" in launchParamSet or "mols(loc)" in launchParamSet
