...
class CEBECalculator:
    ...
    def _print_welcome_message(self):
        ...
        if self.DOLOC: print("\t      Atoms that were localized?:\t", self.ATOMS_TO_LOCALIZE)
    ...
    def _do_rhf(self):
        ...
        if self.doSFX: mf_gs.sfx2c1e()
        mf_gs.kernel()

        if self.toPrintDensity: self.P_GS = mf_gs.make_rdm1() # store density matrix
    ...
    def _do_ccsd(self, triples=False):
        self.mf_gs.do_ccsd(do_ccsd_t=triples, do_t1_diagn=self.DO_T1_DIAGN, verbose=self.VERBOSITY)
    ...
    def _prepare_mol_for_uhf(self):
        ...
        if self.doSFX: umf_core.sfx2c1e()
    ...
    def _do_uhf(self):
        if self.DOLOC:
            ...
            self.umf_core.kernel(mo_coeff0 = np.asarray([self.C_new, self.C_new]), optimizer=self.diis)
            self.umf_core.analyze(self.C_new)
        else:
            self.umf_core.kernel(mo_coeff0 = np.asarray([self.C0,] * 2), optimizer=self.diis)
    ...
    def _do_uccsd(self, triples=False):
        self.umf_core.do_ccsd(do_ccsd_t=triples, do_t1_diagn=self.DO_T1_DIAGN, verbose=self.VERBOSITY, frozen=self.freeze_orbs)

    def run(self):
        self._print_welcome_message()

        if self.doTime: 
            self.timefile.write(f"- HF\n")
            self.timefile.write(f"{self.get_time_now()}\n")
        self._build_mol()
        # --------- RUN RHF --------
        self._do_rhf()
        # --------------------------
        if self.doTime: self.timefile.write(f"{self.get_time_now()}\n")

        if self.doMP2:
            if self.doTime: 
                self.timefile.write(f"- MP2\n")
                self.timefile.write(f"{self.get_time_now()}\n")
            # --------- RUN MP2 --------
            self._do_mp2()
            # --------------------------
        ...
        get_de = lambda x, y: (y - x) * 27.211399
